...
const getAllDeals = async (cid) => {
  ...
  // order- id, dealId, minerId, inclusion proof, verification data
  let tx = await contract.complete(1, 1234, 4321, incProof, verifData, {
    gasLimit: 50_000_000,
  });
  await tx.wait();

  let fileInfo = await contract.getAllDeals(cid, {
    gasLimit: 50_000_000,
  });

  // let GetDetails = await contract.idToUser(fileId);
  console.log("Deals: " + fileInfo);
};
